    ...
    public class SimpleFootsteps : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!isAi)
            {
                ...
                var velocity = _rigidbody ? _rigidbody.velocity : Vector3.zero;
                ...
                speed = 1 - (_rigidbody ? (_rigidbody.velocity.magnitude * playbackSpeedDamping) : 1f);
            }
            else
            {
                //Sensing movement for AI
                if (AIController && AIController.moving)
                {
                    moving = true;
                }
                else
        ...
        IEnumerator senseSteps()
        {
            while (true)
            {
                if (!isAi && PlayerController)
                {
                    if (PlayerController.IsGrounded && moving && !PlayerController.IsSliding)
                    {
                        if (floortag == "grass")
                        {
