...
namespace Mirror
{
    public abstract class NetworkTransformBase : NetworkBehaviour
    {
        ...
        [SyncVar]
        public bool isWorldSpaceTransform;
        ...
        {
            //var posVector = new Vector3(0, 0, 0);
            //var rotVector = new Quaternion(0, 0, 0, 0);

            //if (isWorldSpaceTransform)
            //{
            //    posVector = targetComponent.position;
            //    rotVector = targetComponent.rotation;
            //}
            //if (!isWorldSpaceTransform)
            //{
            //    posVector = targetComponent.localPosition;
            //    rotVector = targetComponent.localRotation;
            //}

            // NetworkTime.localTime for double precision until Unity has it too. Our local time is what the other end uses as remote time, the other end fills out local time itself
            return new NTSnapshot(NetworkTime.localTime, 0, targetComponent.position, targetComponent.rotation, targetComponent.localScale);
        ...
        protected virtual void ApplySnapshot(NTSnapshot interpolated)
        {
            ...
            if (isWorldSpaceTransform)
            {
                if (syncPosition)
                {
                    targetComponent.position = interpolated.position;
                }

                if (syncRotation)
                {
                    targetComponent.rotation = interpolated.rotation;
                }
            }
            if (!isWorldSpaceTransform)
            {
        ...
        void UpdateServer()
        {
            ...
            if (clientAuthority && !hasAuthority)
            {
                if (serverSnapshots.Count > 0)
                {
                    ...
                    if (SnapshotInterpolation.Step(serverSnapshots, Time.unscaledDeltaTime, ref serverTimeline, serverTimescale, Interpolate, out NTSnapshot computed))
                    {
        ...
        void UpdateClient()
        {
            ...
            else
            {
                ...
                {
                    // compute snapshot interpolation & apply if any was spit out
                    if (SnapshotInterpolation.Step(clientSnapshots, Time.unscaledDeltaTime, ref clientTimeline, clientTimescale, Interpolate, out NTSnapshot computed))
                    {
                        ApplySnapshot(computed);
                    }
                }
